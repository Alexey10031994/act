...
function Count() {
    const { count, setCount, setName, name, names, setNames, setDay, day, setMonth, month, months, setYear, year, setPrice1, price1, price2, setPrice2, nav, setNav, order, setOrder, activeEdit, setActivEdit, activeInputEdit, setInputActivEdit } = React.useContext(MyContext);
    ...
    return (
        <>
            ...
            <h2 className='bold-italic margin5'>ФП Севастьянов О.В.</h2>
            ...
            <div className='title'>
...

                {
                    activeInputEdit ?
                        ...
                        <>
                            <input className='input-tac' type="text" onChange={(e) => setName(e.target.value)} value={name} />
                            <img
            ...
            <h2 className='bold-italic letter-sp'>Рахунок №
                <input onChange={(e) => setCount(e.target.value)} value={count} className="input-count" type="text" />
            </h2>
            <p className='tac'>"{day}" {month} {year}р.</p>
                                ...
                                <th>
                                    ...
                                    <p>
                                        {item.clock === '' || item.price === ''
                                            ? ''
                                            : item.countTable = (Number(item.clock) * Number(item.price)).toFixed(2)}
                                    </p>
            ...
            <table>
                <tfoot>
                    ...
                    <tr >
                        <th id='ver' className='tfoot__col-1 bold'>Всього до сплати :</th>
                        <th id='ver' className='tfoot__col-2'>Разом</th>
                        <th className='tfoot__col-3' >
                            {((strsTable.map(item => Number(item.countTable))
                        ...
                        {activeEdit ?
                            <th id='ver' className='tfoot__col-1 relative' >
                                ...
                                {numberInString(
                                    ...
                                    ((strsTable.map(item => Number(item.countTable))
                                        .reduce((summ, count) => summ + count))
                                        .toFixed(2)).slice(0, (((strsTable.map(item => Number(item.countTable))
                                            .reduce((summ, count) => summ + count))
                                            .toFixed(2)).replace('.', ',')).length - 3),

                                    ((strsTable.map(item => Number(item.countTable))
                                        .reduce((summ, count) => summ + count))
                                        .toFixed(2)).slice((((strsTable.map(item => Number(item.countTable))
                                            .reduce((summ, count) => summ + count))
                                            .toFixed(2)).replace('.', ',')).length - 2,
                                            (((strsTable.map(item => Number(item.countTable))
                                                .reduce((summ, count) => summ + count))
                                                .toFixed(2)).replace('.', ',')).length
                                        )

                                )}

                                <img onClick={() => setActivEdit(!activeEdit)} className='absolute' src="./icon/editing.png" alt="edit" width={16} />
                            </th> :

                            <th id='ver' className='tfoot__col-1 relative'>
                                <input className='table__input-text' type="text" />
                                <img onClick={() => setActivEdit(!activeEdit)} className='absolute' src="./icon/close.png" alt="edit" width={16} />

                            </th>
                        }

                        <th id='ver' className='tfoot__col-2'>ПДВ</th>
                        <th className='tfoot__col-3' colSpan="2">--------------</th>
                    </tr>
                    <tr >
                        <th id='ver' className='tfoot__col-1'></th>
                        <th id='ver' className='tfoot__col-2'>Всього :</th>
                        <th className='tfoot__col-3' >
                            {((strsTable.map(item => Number(item.countTable))
                                .reduce((summ, count) => summ + count))
                                .toFixed(2)).replace('.', ',')}
